...
import { b as $format, c as $locale } from './server_C1vOnolk.mjs';
import { I as InView } from './InView_56LALd20.mjs';
import 'clsx';
import '@formatjs/intl-segmenter/polyfill.js';
/* empty css                         */
import { b as aboutContent } from './index_US5N2CrH.mjs';
...
function AboutSect($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		let { locale = "en", data: dataProp } = $$props;
		const lang = locale === "gr" ? "gr" : "en";
		const data = dataProp ?? aboutContent[lang];
...
function ProjectsSect($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		var $$store_subs;
		let { locale = "en", projects: projectsProp } = $$props;
		const t = $format;
		const lang = store_get($$store_subs ??= {}, '$localeStore', $locale) === "gr" ? "gr" : "en";
		const featuredProjects = projectsProp ?? getFeaturedProjectsByLanguage(lang);
